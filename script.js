function printpdf() {
    ...
    // Show buttons and checkboxes after printing
    allButtons.forEach(button => {
...
function addedu() {
    ...
    head.innerHTML = ('<div class="edublock"><span><input type="checkbox" class="input-checkbox"></span><span class="education-head" contenteditable="true">YOUR DEGREE</span><div ><span contenteditable="true">Institute name</span> - <span contenteditable="true">Passing Year</span> - <span contenteditable="true">Marks</span></div></div>');
    saveresume();
  }
...
function addProject() {
    const head = document.createElement('div');
    document.getElementById("projectsList").appendChild(head);
    head.innerHTML = ('<div class="project"><span><input type="checkbox" class="input-checkbox"></span><span contenteditable="true">Write your project details</span></div>');
    saveresume();
}

function remProject(event) {
    let val = 0;
    const allInputCheckboxes = event.target.parentElement.getElementsByClassName("input-checkbox");
    const array = Array.from(allInputCheckboxes);
    if (array.length === 0) {
...
function addsec() {
  if (maxNewSection < 2) {
      ...
      if (maxNewSection === 0) {
          head.innerHTML = ('<div><span><input type="checkbox" class="input-checkbox"></span><span class="title" contenteditable="true">NEW SECTION</span><br><br><div contenteditable="true">Explain your Topic.</div></div>');
      }
      else {
          head.innerHTML = ('<div><br><br><span><input type="checkbox" class="input-checkbox"></span><span class="title" contenteditable="true">NEW SECTION</span><br><br><div contenteditable="true">Explain your Topic.</div></div>');
      }
